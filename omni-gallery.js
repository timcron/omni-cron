...
`;
  ...
  function initCarousel(block) {
    ...
    function positionCards() {
      const minSide = Math.min(block.clientWidth, block.clientHeight);
      let r_eff = parseUnit(cfg.radius).v;
      if (parseUnit(cfg.radius).u === '%') r_eff = minSide * (r_eff / 100);
      ...
      rowsContainers.forEach((rDiv, rIdx) => {
        ...
        rowCards.forEach((card, i) => {
          ...
          const y = (rIdx - (rows - 1) / 2) * (parseFloat(cfg.cardHeight) * cfg.rowGap);
          ...
          card.style.opacity = 0.5 + (z / r_eff + 1) / 4;
        });
      });
    }
