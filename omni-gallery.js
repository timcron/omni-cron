(function() {
  ...
  function ensureInjectedStyles() {
    if (document.getElementById('omni-3d-styles')) return;
    const css = `
      ...
      .omni-3d-carousel .omni-3d-inner { position:absolute !important; inset:0 !important; transform-style: preserve-3d !important; display:flex !important; align-items:center !important; justify-content:center !important; pointer-events:none !important; will-change: transform; }
      ...
      .omni-3d-carousel .omni-card--mounted { position:absolute !important; top: 50% !important; left:50% !important; transform-style: preserve-3d !important; transform-origin:center center !important; pointer-events:auto !important; transition: transform 400ms cubic-bezier(.2,.9,.3,1) !important; will-change: transform; backface-visibility: hidden; cursor:grab !important; display:block !important; margin:0 !important; }
      .omni-3d-carousel .omni-card--mounted:active { cursor:grabbing !important; }
      .omni-3d-carousel .omni-card--mounted img { display:block !important; width:100% !important; height:100% !important; object-fit:cover !important; pointer-events:none !important; user-select:none !important; -webkit-user-drag:none !important; }
    `;
    const style = document.createElement('style');
    style.id = 'omni-3d-styles';
  ...
  function initCarousel(block) {
    ...
    cards.forEach(el => {
      if (!inner.contains(el)) {
        ...
        preventImageDrag(el.querySelector('img'));
    ...
    function updatePosition() {
      // Core 3D engine logic adapted for OMNI system
      const rows = block.querySelectorAll('.omni-3d-row');
      rows.forEach((row, rIdx) => {
        const rowCards = row.querySelectorAll(cfg.cardSelector);
        rowCards.forEach((card, i) => {
          // Logic for transform3d positioning
        });
      });
    }
  }
